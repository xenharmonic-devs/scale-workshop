...
describe('Scale store', () => {
  ...
  it('deserializes 5edo', () => {
    const data = JSON.parse(SERIALIZED, (key: string, value: any) =>
      RootContext.reviver(key, Interval.reviver(key, value))
    )
    data.id = '123abcABC'
    const scale = useScaleStore()
    scale.fromJSON(data)
  ...
  it('recovers legacy data', () => {
    const data = JSON.parse(LEGACY_SERIALIZED, (key: string, value: any) =>
      RootContext.reviver(key, Interval.reviver(key, value))
    )
    data.id = '123abcABC'
    const scale = useScaleStore()
