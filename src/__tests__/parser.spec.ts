...
describe("Line parser", () => {
  ...
  it("parses N-of-EDO (negative)", () => {
    ...
    expect(result.lineType).toBe("equal temperament");
  });
  ...
  it("parses composites (negative offset)", () => {
    ...
    expect(result.name).toBe("3/2 - 1.955");
    expect(result.lineType).toBe("ratio");
    result.monzo.cents = 0;
    const expected = new ScaleLine(
  ...
  it("parses composites (any)", () => {
    ...
    expect(result.lineType).toBe("any");
