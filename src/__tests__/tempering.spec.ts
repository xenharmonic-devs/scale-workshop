...
describe('Temperament Mapping', () => {
  it('calculates POTE meantone', () => {
    ...
    const syntonicComma = new Interval(ExtendedMonzo.fromFraction(new Fraction(81, 80), 3), 'ratio')
    const octave = new Interval(ExtendedMonzo.fromFraction(2, 3), 'ratio')
    const fifth = new Interval(ExtendedMonzo.fromFraction(new Fraction(3, 2), 3), 'ratio')
  ...
  it('can temper out multiple commas', () => {
    ...
    const marvelComma = new Interval(ExtendedMonzo.fromFraction(new Fraction(225, 224), 4), 'ratio')
    const gamelisma = new Interval(ExtendedMonzo.fromFraction(new Fraction(1029, 1024), 4), 'ratio')
    ...
    const largeSecor = new Interval(ExtendedMonzo.fromFraction(new Fraction(16, 15), 4), 'ratio')
    const smallSecor = new Interval(ExtendedMonzo.fromFraction(new Fraction(15, 14), 4), 'ratio')
  ...
  it('can temper to rank 1', () => {
    ...
    const octave = new Interval(ExtendedMonzo.fromFraction(2, 3), 'ratio')
    const syntonicComma = new Interval(ExtendedMonzo.fromFraction(new Fraction(81, 80), 3), 'ratio')
    const diesis = new Interval(ExtendedMonzo.fromFraction(new Fraction(128, 125), 3), 'ratio')
    const semitone = new Interval(ExtendedMonzo.fromFraction(new Fraction(16, 15), 3), 'ratio')
    expect(tet12.apply(syntonicComma).totalCents()).toBeCloseTo(0)
    expect(tet12.apply(diesis).totalCents()).toBeCloseTo(0)
    expect(tet12.apply(semitone).totalCents() * 12).toBeCloseTo(tet12.apply(octave).totalCents())
  ...
  it('can temper to rank 3', () => {
    ...
    const octave = new Interval(ExtendedMonzo.fromFraction(2, 4), 'ratio')
    const breedsma = new Interval(ExtendedMonzo.fromFraction(new Fraction(2401, 2400), 4), 'ratio')
  ...
  it('leaves primes outside of the subgroup alone', () => {
    ...
    const largeLimma = new Interval(ExtendedMonzo.fromFraction(new Fraction(27, 25), 5), 'ratio')
    const minorSixth = new Interval(ExtendedMonzo.fromFraction(new Fraction(11, 7), 5), 'ratio')
    expect(bug.apply(largeLimma).totalCents()).toBeCloseTo(0)
    expect(bug.apply(minorSixth).monzo.toFraction().equals(new Fraction(11, 7))).toBeTruthy()
  })
  ...
  it('can construct POTE mavila from a list of vals', () => {
    ...
    const fifth = new Interval(ExtendedMonzo.fromFraction(new Fraction(3, 2), 4), 'ratio')
    const seventh = new Interval(ExtendedMonzo.fromFraction(new Fraction(7, 4), 4), 'ratio')
    ...
    expect(mavila.apply(seventh).monzo.toFraction().equals(new Fraction(7, 4))).toBeTruthy()
  })
  ...
  it('can construct CTE septimal meantone from a list of vals', () => {
    ...
    const syntonicComma = new Interval(ExtendedMonzo.fromFraction(new Fraction(81, 80), 4), 'ratio')
    const starlingComma = new Interval(
      ExtendedMonzo.fromFraction(new Fraction(126, 125), 4),
      'ratio'
    )
    const octave = new Interval(ExtendedMonzo.fromFraction(2, 4), 'ratio')
    const tritave = new Interval(ExtendedMonzo.fromFraction(new Fraction(3, 1), 4), 'ratio')
    const pentave = new Interval(ExtendedMonzo.fromFraction(new Fraction(5, 1), 4), 'ratio')
    const seven = new Interval(ExtendedMonzo.fromFraction(new Fraction(7, 1), 4), 'ratio')
  ...
  it('can be applied to scales', () => {
    ...
    const neutralSecond = new Interval(ExtendedMonzo.fromFraction(new Fraction(12, 11), 5), 'ratio')
    const submajorSecond = new Interval(
      ExtendedMonzo.fromFraction(new Fraction(11, 10), 5),
      'ratio'
    )

    const porkypine8 = porkypine.apply(Scale.fromRank2(neutralSecond, octave, 8, 0))
    ...
    expect(porkypine8.getMonzo(7).totalCents()).toBeGreaterThan(porkypine8.getMonzo(6).totalCents())
