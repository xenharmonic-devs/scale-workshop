...
<template>
  <div class="control-group">
    ...
    <div class="control radio-group">
      ...
      <span>
        <input type="radio" id="colors-silver" value="silver" v-model="scale.autoColors" @input="updateScale"/>
        <label for="colors-silver"> Silver </label>
      </span>

      <span>
        <input type="radio" id="colors-cents" value="cents" v-model="scale.autoColors" @input="updateScale"/>
        <label for="colors-cents"> Cents </label>
      </span>

      <span>
        <input type="radio" id="colors-factors" value="factors" v-model="scale.autoColors" @input="updateScale"/>
  ...
  <div class="control-group">
    <h2>Scale data</h2>
    <div class="control">
      <textarea ref="sourceEditor" rows="20" v-model="scale.sourceText" @input="updateScale" @focus="clearPaletteInfo"></textarea>
    </div>
    <ScaleRule :scale="scale.scale" />
    <p class="error">{{ scale.error }}</p>
    <h3>Character palette</h3>
    <div class="control">
      <button v-for="_, character, i of palette" :key="i" @click="insertFromPalette" @mouseenter="updatePaletteInfo">{{ character }}</button>
    </div>
