...
export function getTestData(appTitle: string) {
  const intervals = [
    new ScaleLine(ExtendedMonzo.fromCents(100, 3), "cents"),
    new ScaleLine(
    ...
    new ScaleLine(ExtendedMonzo.fromFraction(new Fraction(5, 3), 3), "ratio"),
    new ScaleLine(ExtendedMonzo.fromValue(Math.E / 2, 3), "decimal"),
    new ScaleLine(
      ExtendedMonzo.fromFraction(new Fraction(81, 80), 3).mul(5),
      "ratio"
    ),
    new ScaleLine(ExtendedMonzo.fromNumber(2, 3), "ratio"),
  ];
